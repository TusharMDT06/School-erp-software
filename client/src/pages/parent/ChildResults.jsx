...
const ChildResults = () => {
  ...
  useEffect(() => {
    if (selectedChildId) {
      dispatch(fetchStudentResults(selectedChildId));
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Child Examination Results</h2>
          <p className="text-xs text-slate-500 mt-1">
            Review your child's published marks, AI teacher appraisals, and download official signed PDF report cards.
          </p>
        </div>
      ...
      {loadingChildren || loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-16 text-center text-slate-400 text-xs">
          No published examination results available for {selectedChild?.userId?.name || "this student"} yet.
        </div>
      ) : (
              ...
              >
                ...
                >
                  <div className="flex items-start gap-3.5">
                    <div className="w-11 h-11 rounded-2xl bg-amber-50 text-amber-700 flex items-center justify-center font-bold text-sm flex-shrink-0">
                      <Award className="w-6 h-6" />
                    </div>
                    <div>
                      <h3 className="text-base font-bold text-slate-800">{exam.examName}</h3>
                      <p className="text-xs text-slate-400 mt-0.5">
                        Child: <strong>{selectedChild?.userId?.name}</strong> | Academic Year:{" "}
                  ...
                  <div className="flex items-center gap-4 self-end md:self-auto">
                    <div className="text-right">
                      <p className="text-xs text-slate-400">Aggregate Score</p>
                    ...
                    <a
                      href={`${import.meta.env.VITE_API_URL || "http://localhost:5000/api"}/results/reportcard/${selectedChild?._id}/${exam._id}`}
                      target="_blank"
                      rel="noreferrer"
